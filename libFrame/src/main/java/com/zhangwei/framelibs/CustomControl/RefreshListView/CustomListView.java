...
//    private ProgressBar progressBar;
    ...
    private void Init(Context context) {
        ...
        headView = (LinearLayout) inflater.inflate(R.layout.custom_pull_down_head,
                null);

        headView.setBackgroundColor(getResources().getColor(R.color.bg_transparency));
        arrowImageView = (ImageView) headView
                .findViewById(R.id.head_arrowImageView);
        animationDrawable = (AnimationDrawable) arrowImageView.getBackground();
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {   //手指正在移动的时候
                ...
                if (showRefresh && getFirstVisiblePosition() == 0) {
                    ...
                    if (state != REFRESHING && isRecored && state != LOADING) {
                        ...
                        if (state == RELEASE_To_REFRESH) {
                            ...
                            if (((tempY - moveY) / RATIO < headContentHeight)
                                    && (tempY - moveY) > 0) {
                        ...
                        if (state == PULL_To_REFRESH) {
                            headView.setPadding(0, -1 * headContentHeight
                                    + (tempY - moveY) / RATIO, 0, 0);
                        }
                        // 更新headView的paddingTop
                        if (state == RELEASE_To_REFRESH) {
                            headView.setPadding(0, (tempY - moveY) / RATIO
                ...
                if (firstVisiblePosition + childCount >= itemCount
                    ...
                    final boolean isHandleOnListViewBottomAndPullDown = mOnScrollOverListener
...
//                lastUpdatedTextView.setVisibility(View.VISIBLE);
     ...
     * 滚动监听接口
     *
     * @see CustomListView#setOnScrollOverListener(OnScrollOverListener)
     */
